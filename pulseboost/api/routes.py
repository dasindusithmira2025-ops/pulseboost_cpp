...
async def benchmark_result_export(request: Request, benchmark_id: str) -> str:
    ...
    lines = [
        ...
        "",
        "## Comparison",
        metric_line("Average FPS", "avg_fps", "avg_fps", "avg_fps_delta"),
        metric_line("1% Low FPS", "one_percent_low_fps", "one_percent_low_fps", "one_percent_low_delta"),
        f"- Average frame time: baseline={(result.get('baseline') or {}).get('average_frame_time_ms')} ms, optimized={(result.get('optimized') or {}).get('average_frame_time_ms')} ms",
        f"- P95 frame time: baseline={(result.get('baseline') or {}).get('p95_frame_time_ms')} ms, optimized={(result.get('optimized') or {}).get('p95_frame_time_ms')} ms, delta={result.get('p95_frame_time_delta_percent')}%",
        ...
        "",
        "## Frame-Time Evidence",
        f"- Supported: {result.get('frametime_supported')}",
        f"- Source: {result.get('frametime_source')}",
        f"- Reason: {result.get('frame_time_reason')}",
        "",
        "## Tweaks",
    ]
