...
class BenchmarkEngine:
    ...
    async def run(
        ...
        *,
        ...
        try:
            baseline = await self._capture_window(duration_seconds)
            ...
            optimized = await self._capture_window(duration_seconds)
    ...
    async def _capture_window(self, duration_seconds: int) -> BenchmarkCapture:
        sample_count = max(2, int(duration_seconds))
        cpu_samples: list[float] = []
        unsupported_reasons = [
            "Average FPS capture is unavailable because no frame hook or game telemetry integration is connected in the current desktop runtime.",
            "1% low FPS capture is unavailable without a game/frame hook integration.",
            "Frame-time variance is unavailable without frame capture support.",
        ...
        for index in range(sample_count):
            cpu_samples.append(self._sample_cpu_percent())
    ...
    def _sample_cpu_percent(self) -> float:
        if not isinstance(self.collector, MetricCollector):
            snapshot = self.collector.get_snapshot()
            return float(snapshot.cpu_total)
        try:
